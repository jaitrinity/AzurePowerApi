...
// $sql="SELECT m.Category, m.SubCategory, m.Caption, m.CheckpointId, ma.ActivityId FROM Mapping ma join Menu m on ma.MenuId=m.MenuId where ma.IR_Id=$irId";
// $sql="SELECT m.Category, m.SubCategory, m.Caption, m.CheckpointId, ma.ActivityId, (case when m.SampleSize is null then ir.SampleSize else m.SampleSize end) as SampleSize FROM Mapping ma join Menu m on ma.MenuId=m.MenuId left join InsReqMaster ir on ma.IR_Id=ir.IR_Id where ma.IR_Id=$irId";
