...
// 		m.Caption,m.SubCategory,m.Category,m.Icons,m.CheckpointId,m.Colors
// 		FROM Mapping mp 
// 		left join Menu m  on (mp.MenuId = m.MenuId)
// 		left join Location l on (mp.LocationId = l.LocationId)
// 		WHERE mp.EmpId = '$empId' AND date(mp.StartDate) <= date(now()) AND date(mp.EndDate) >= date(now())
// 		AND mp.ActivityId = 0";

$assignSql = "SELECT mp.MenuId,mp.LocationId,mp.StartDate,mp.EndDate,mp.MappingId,l.Name,l.GeoCoordinates,
		m.Caption,m.SubCategory,m.Category,m.Icons,m.CheckpointId,m.Colors,mp.IR_Id, (case when m.SampleSize is not null then m.SampleSize else ir.SampleSize end) as SampleSize
		FROM Mapping mp 
		left join Menu m  on (mp.MenuId = m.MenuId)
		left join Location l on (mp.LocationId = l.LocationId)
